#!/usr/bin/env python3
"""
Headless: POST Budi's MP3 to /api/transcribe (and optionally /api/query).
Same multipart as the browser; no UI. For full mic + waveform + /chat, use:
  npm run demo:voice-e2e:budi
  (see docs/DEMO_BUDI.md)

Usage (from repo root, backend running on :8000):
  python scripts/demo_budi.py
  python scripts/demo_budi.py --full-query --persona migrant
...
def find_default_mp3(repo_root: Path) -> Path | None:
    for p in repo_root.glob("ElevenLabs_*Budi*.mp3"):
        return p
    alt = repo_root / "public" / "demo" / "budi.mp3"
    if alt.is_file():
        return alt
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Budi voice → transcribe (+ optional query)")
    parser.add_argument(
        "--audio",
        type=Path,
        help="Path to MP3 (default: ElevenLabs_*Budi*.mp3 in repo root or public/demo/budi.mp3)",
    )
    ...
    parser.add_argument(
        ...
        help="Persona for /api/query (PRD: Budi → migrant worker)",
    )
    ...
    if audio_path is None:
        ...
        if not found:
            print(
                "No MP3 found. Place ElevenLabs_*Budi*.mp3 in repo root or run scripts/copy_budi_demo_audio.ps1",
                file=sys.stderr,
            )
            return 1
        audio_path = found
